...
Во время активного матча live-значения game clock и shot clock рассчитывает Timer Service. `MATCH_CLOCK_STATE` хранит последний подтверждённый снимок для восстановления, UI и аудита. Node.js backend обновляет эту таблицу после подтверждения команды времени и записи события в `MATCH_EVENT`.
...
1. Получить команду от панели, пульта или Timer Service.
2. Проверить команду по статусу матча, периоду, ограничениям FIBA и выбранному игроку/команде.
3. Для команд времени принять версионированный clock snapshot от Timer Service.
4. Вставить одну запись `MATCH_EVENT`.
5. Вставить детальную запись события, если она нужна.
6. Обновить `MATCH_CLOCK_STATE`, `MATCH_TEAM_STATE`, `MATCH_TIMEOUT_BUCKET`, `MATCH_PLAYER_STAT` или `MATCH_PERIOD_SCORE` по типу события.
7. Зафиксировать транзакцию.
8. Разослать зафиксированное состояние клиентам и подтвердить версию Timer Service.
...
5. Передать последний подтверждённый clock snapshot в Timer Service.
6. Возобновить таймеры по правилам monotonic timing Timer Service.
7. Разослать восстановленное состояние на LED-экран и операторские клиенты.
