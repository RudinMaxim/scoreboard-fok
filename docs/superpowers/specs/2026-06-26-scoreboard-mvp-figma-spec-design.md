...
- game clock оформлен как нейтральный блок `ИГРОВОЕ ВРЕМЯ / текущее значение / МИН:СЕК`;
- игровое время не использует красный фон, чтобы не конкурировать с таймером атаки;
- shot clock выделен danger/accent цветом;
- shot clock оформлен как вертикальный блок `АТАКА / текущее значение / СЕК`, чтобы зритель не путал его с game clock;
- текущее значение shot clock остаётся крупнейшим элементом внутри блока;
...
- технические статусы `WebSocket`, `Timer Service` и `Backend` не показываются на зрительском `LED / Game`; они доступны только в `Control / System Status`;
